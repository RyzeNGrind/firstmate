...
# Optional 5th arg: pool_root - when set, passes TREEHOUSE_ROOT=<pool_root> to treehouse
# so it finds the right per-home pool; absent or empty means default pool discovery.
teardown_treehouse_return() {
  local dir=$1 cd_dir=$2 label=$3 post_cleanup_check=${4:-} pool_root=${5:-}
  local out lock attempt=0 max_retries lock_desc
  local treehouse_env=()
  [ -z "$pool_root" ] || treehouse_env=(env "TREEHOUSE_ROOT=$pool_root")

  # Capture stdout+stderr so non-lock failures stay visible and lock failures can
  # be matched by signature even when the lock file is already gone mid-check.
  if out=$( ( cd "$cd_dir" && "${treehouse_env[@]+"${treehouse_env[@]}"}" treehouse return --force "$dir" ) 2>&1 ); then
    [ -n "$out" ] && printf '%s\n' "$out"
    return 0
  fi
  [ -n "$out" ] && printf '%s\n' "$out" >&2
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ "$child_owner_rc" -eq "$TEARDOWN_SLOT_REASSIGNED_RC" ]; then
        ...
        if [ -n "$child_proj" ] && [ -d "$child_proj" ] && command -v treehouse >/dev/null 2>&1; then
          if teardown_treehouse_return "$child_wt" "$child_proj" "child worktree" "" "$(meta_value "$child_meta" treehouse_pool_root)"; then
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  # to pool. treehouse resolves the pool from the working directory (and the per-home
  # pool root passed as the 5th arg); run it from the project. teardown_treehouse_return
  # tolerates transient and stale git locks left by a killed crew process; see the script
  # header for retry and stale-lock proof.
  post_lock_cleanup_check=
  if [ "$FORCE" != "--force" ] && [ "$KIND" != scout ] && [ "$KIND" != secondmate ]; then
    post_lock_cleanup_check=validate_worktree_teardown_safety
  fi
  teardown_treehouse_return "$WT" "$PROJ" "worktree" "$post_lock_cleanup_check" "$(meta_value "$META" treehouse_pool_root)" || {
